...
if [ $? -ne 0 ]; then
    if [ "$remi" = 'yes' ]; then
        yum -y --disablerepo=* \
            --enablerepo="*base,*updates,bitrix,epel,vesta,remi*" \
            install $software
    else
        yum -y --disablerepo=* --enablerepo="*base,*updates,bitrix,epel,vesta" \
            install $software
    fi
fi
...
if [ "$nginx" = 'yes' ]; then
    ...
    sed "s/%SERVER_IP%/${ip}/g" -i /etc/nginx/bx/settings/rtc-im_settings.conf
    cp -f $vestacp/nginx/bx/conf/* /etc/nginx/bx/conf/
    cp -f $vestacp/nginx/rtc-server.conf /etc/nginx/conf.d/
...
if [ "$mysql" = 'yes' ]; then
    ...
    cp -f $vestacp/$service/mysql.conf /etc/my.cnf
